...
def get_widget_data(plan_id):
    ...
    добавленого датчика DS18D20.

    :param plan_id: int ID планировки.
    :returns: list Список кортежей с данными температуры и координатами расположения
    виджета.
    """

    sensors = get_used_plugins_by(package='plugins.arduino_ds18d20')
    ...
    return [(plan_id, v.content_object.name, v.content_object.horiz_position,
             ...
             v.temperature) for v in values if v is not None]
